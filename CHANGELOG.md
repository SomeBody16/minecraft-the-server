{:outlink:target="blank" rel="noopener noreferrer"}

## 1.4.0 [#45](https://github.com/SomeBody16/minecraft-the-server/pull/45/changes){:outlink}

- Colossal chests maxSize: 5 -> 10
- Added steampunk textures for mods:
    - Applied Energetics
    - Building Gadgets
    - Camera
    - Computer Craft
    - Easy Villagers
    - Immersive Aircrafts
    - Simple Storage Network
    - Thermal Series

---

## 1.3.0 [#44](https://github.com/SomeBody16/minecraft-the-server/pull/44/changes){:outlink}

- Added recipe for corundum
- Added addiction to crystal
- Fixed some translations and textures
- Hammer trident model [WIP]

#### Added mods:

- [Create: Broken Bad](https://www.curseforge.com/minecraft/mc-mods/create-broken-bad){:outlink}
- [Ghostwriter](https://github.com/waffle-stomper/Ghostwriter){:outlink}
- [MOA DECOR: BATH](https://www.curseforge.com/minecraft/mc-mods/moa-decor-bath){:outlink}

## 1.2.0 [#43](https://github.com/SomeBody16/minecraft-the-server/pull/43/changes){:outlink}

- Wold's Vaults 0.25.3 -> 0.25.4 [CHANGELOG](https://www.curseforge.com/minecraft/modpacks/wolds-vaults/files/7389645){:outlink}
- Vault Dwellers will now have player skins

#### Added mods:

- [Snowy Spirit](https://www.curseforge.com/minecraft/mc-mods/snowy-spirit){:outlink}
- [AlcoCraft+](https://modrinth.com/mod/alcocraft?version=1.18.2&loader=forge){:outlink}
- [Create Slice & Dice](https://modrinth.com/mod/slice-and-dice?version=1.18.2&loader=forge){:outlink}
- [Brewin' And Chewin'](https://modrinth.com/mod/brewin-and-chewin?version=1.18.2&loader=forge){:outlink}

## 1.1.0 [#42](https://github.com/SomeBody16/minecraft-the-server/pull/42/changes){:outlink}

- Wold's Vaults 0.25.2 -> 0.25.3 [CHANGELOG](https://www.curseforge.com/minecraft/modpacks/wolds-vaults/files/7373941){:outlink}
- Vault Hunters 3.20.2 -> 3.20.3 [CHANGELOG](https://discord.com/channels/889424759018901514/1048680576988762182/1453434437869244528){:outlink}

- Removed mods: [Areas](https://www.curseforge.com/minecraft/mc-mods/areas){:outlink}, [Block Runner](https://www.curseforge.com/minecraft/mc-mods/block-runner){:outlink}, [Chat Heads](https://www.curseforge.com/minecraft/mc-mods/chat-heads){:outlink}, [Dynamic FPS](https://www.curseforge.com/minecraft/mc-mods/dynamic-fps){:outlink}, [Incendium](https://www.curseforge.com/minecraft/mc-mods/incendium){:outlink}, [Nullscape](https://www.curseforge.com/minecraft/mc-mods/nullscape){:outlink}

## 1.0.1

- Fixed missing configs

## 1.0.0

- Wold's Vaults 0.25.2
- Vault Hunters 3.20.2

#### Added mods:

- [Security Craft](https://www.curseforge.com/minecraft/mc-mods/security-craft){:outlink}
- [Block Runner](https://www.curseforge.com/minecraft/mc-mods/block-runner){:outlink}
- [Callable Horses](https://www.curseforge.com/minecraft/mc-mods/callable-horses){:outlink}
- [Chat Heads](https://www.curseforge.com/minecraft/mc-mods/chat-heads){:outlink}
- [Cosmetic Armor Reworked](https://www.curseforge.com/minecraft/mc-mods/cosmetic-armor-reworked){:outlink}
- [Dynamic FPS](https://www.curseforge.com/minecraft/mc-mods/dynamic-fps){:outlink}
- [Entity Culling](https://www.curseforge.com/minecraft/mc-mods/entity-culling){:outlink}
- [Farmers Delight](https://www.curseforge.com/minecraft/mc-mods/farmers-delight){:outlink}
- [Incendium](https://www.curseforge.com/minecraft/mc-mods/incendium){:outlink}
- [Not Enough Animations](https://www.curseforge.com/minecraft/mc-mods/not-enough-animations){:outlink}
- [Nullscape](https://www.curseforge.com/minecraft/mc-mods/nullscape){:outlink}
- [Sound Physics](https://www.curseforge.com/minecraft/mc-mods/sound-physics-remastered){:outlink}
- [SWEM](https://www.curseforge.com/minecraft/mc-mods/swem){:outlink}
- [Areas](https://www.curseforge.com/minecraft/mc-mods/areas){:outlink}
- [Custom Player Models](https://www.curseforge.com/minecraft/mc-mods/custom-player-models){:outlink}
- [Infinity Buttons](https://www.curseforge.com/minecraft/mc-mods/infinitybuttons){:outlink}
- [Macaw's Holidays](https://www.curseforge.com/minecraft/mc-mods/macaws-holidays){:outlink}
- [MOA decor Holidays](https://www.curseforge.com/minecraft/mc-mods/moa-decor-holidays){:outlink}
- [Gauges and Switches](https://www.curseforge.com/minecraft/mc-mods/redstone-gauges-and-switches){:outlink}
- [The Nexus](https://www.curseforge.com/minecraft/mc-mods/the-nexus-deep-dark-dimension){:outlink}
- [WorldEdit](https://www.curseforge.com/minecraft/mc-mods/worldedit){:outlink}
