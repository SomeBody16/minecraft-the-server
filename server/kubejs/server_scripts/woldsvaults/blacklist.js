...
function onBlacklistItem(event, entity, item) {
    ...
    if (MENTION_ITEMS) entity.tell(formattedMessage("Item", item.id));
...
function onBlacklistBlock(event, entity, block) {
    ...
    if (MENTION_BLOCKS) entity.tell(formattedMessage("Block", block.id));
}
...
function blackListCheck(event, id) {
    ...
    // on the blacklist.
    if (!global.BLACK_LIST[id]) return false;
    // If there is some custom logic behind the blacklist,
    // pass the event through so the custom function can handle it.
    return typeof global.BLACK_LIST[id] == "function" ? global.BLACK_LIST[id](event) : true;
}
...
function basicItemEvent(event) {
    if (blackListCheck(event, event.item.id)) {
        ...
        if (event.canCancel()) event.cancel();
        if (event.item != null && event.item != undefined) event.item.count = 0;
    }
}
...
function basicEntityEvent(event) {
    ...
    if (blackListCheck(event, event.entity.type)) {
        ...
        if (MENTION_ENTITIES) event.server.tell(formattedMessage("Entity Type", event.entity.type));
        // Remove the entity.
        event.entity.remove();
        ...
        if (blackListCheck(event, event.entity.item.id)) {
            ...
            if (MENTION_ENTITIES)
                event.server.tell(formattedMessage("Item Entity", event.entity.item.id));
