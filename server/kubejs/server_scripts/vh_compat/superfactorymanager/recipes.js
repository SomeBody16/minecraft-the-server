let removedOutputsSFM = ["sfm:manager", "sfm:cable", "sfm:disk", "sfm:labelgun"];
onEvent("recipes", (event) => {
    removedOutputsSFM.forEach((id) => {
        event.remove({ output: `${id}` });
    });

    event.shaped(Item.of("sfm:manager"), ["CBC", "BRB", "CBC"], {
        C: "the_vault:chromatic_steel_block",
        B: "sfm:cable",
        R: "the_vault:echo_pog",
    });

    event.shaped(Item.of("sfm:cable", 16), ["CGC", "BRB", "CGC"], {
    ...
    event.shaped(Item.of("sfm:disk"), ["CGC", "BRX", "LGL"], {
    ...
    event.shaped(Item.of("sfm:labelgun"), [" GX", " CB", "C  "], {
